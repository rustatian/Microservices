...
func NewServer(
	...
	s := &Server{
		...
		QueueSize:     QueueSize,
		QueueCh:       make(chan workerSrtuct), //QueueCh
		ErrorCh:       ErrorCh,
	}
	for _, option := range options {
		option(s)
...
func (s *Server) MsgHandler(msg workerSrtuct) {
	s.QueueCh <- msg
}
...
func (s *Server) StartCollectLogs(stop chan os.Signal) {
	...
	s.QueueCh = make(chan workerSrtuct, s.QueueSize)
...
func (s *Server) worker(id int) {
	...
	for {
		...
		case payload, opened := <-s.QueueCh:
			...
			s.flush(id, payload, "max size reached")
			//buffer = nil
...
func (s *Server) flush(workerId int, buffer workerSrtuct, reason string) {
	...
	ctx := buffer.r.Context()

	if s.finalizer != nil {
		iw := &interceptingWriter{buffer.w, http.StatusOK, 0}
		defer func() {
			ctx = context.WithValue(ctx, ContextKeyResponseHeaders, iw.Header())
			ctx = context.WithValue(ctx, ContextKeyResponseSize, iw.written)
			s.finalizer(ctx, iw.code, buffer.r)
		}()
		buffer.w = iw
	}
	...
	request, err := s.dec(ctx, buffer.r)
	if err != nil {
		s.logger.Log("err", err)
		s.errorEncoder(ctx, err, buffer.w)
		return
	}
	...
	if err := s.enc(ctx, buffer.w, response); err != nil {
		s.logger.Log("err", err)
		s.errorEncoder(ctx, err, buffer.w)
		return
	}
